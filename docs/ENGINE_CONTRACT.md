...
| `tools\scripts\start.ps1:52` | `$ENGINE_PORT = 9880`，只起一个引擎。⛔ 补记【修订 1】：它也**没有起第二台引擎的能力** —— 全文 grep 不到 `shim`/`9881`/`indextts2`，而名片的 `runtime` 段（`python`/`entry`/`ready_endpoint`）**全平台无人读**。所以今天 IndexTTS2 的进程要人手工拉起来 |
...
| 字段 | 谁该读它 | 不读的后果 |
|---|---|---|
| `runtime`（`python` / `entry` / `checkpoints` / `ready_endpoint` / `ready_timeout_ms` / `preload`） | 进程管理（**尚不存在**） | ⛔ **引擎进程没人拉起来**，要人手工敲命令。"扔进来就能用"这句话卡在这里 |
| `capabilities.supports_finetune` | 训练页显隐 | `profile.js:247` **解析出来了，全仓没有任何消费者** ⇒ 契约里"false 就不显示训练页"这句话目前是空头支票 |

**"把名片接上电"不是从零设计，是把已经写对的东西接上。**

⚠ 顺带一条**契约本身的缺口**（不是代码的账，待 Owner 拍板后补条款）：
名片里 `max_chars` / `timeout_ms` / `runtime.ready_timeout_ms` 三个值，
IndexTTS2 那张名片的注释写明它们是**按实测推算的**（RTF≈32、冷启动
import 34.55s + 构造 26.51s）。⇒ 一个刚接引擎的人**填不出这三个数**，
除非他先跑一轮。契约目前没有任何一节说这三个数该怎么来 ——
要么允许留空由平台保守取值，要么由冒烟那一步实测回填。
