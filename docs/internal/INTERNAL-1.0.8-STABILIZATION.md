...
> 本文件是内部工作计划，不代表已发布版本。
>
> 目标：在“基本可用、关键流程可验收、可回滚、可维护”的门槛达到之前，不合并到 `main`，不 push `main`，不 release，不重新创建正式 `v1.0.8` tag。`dev` 可以在明确授权下 push 作为冷备份，但不视为稳定发布。
...
当前阶段以保持有效迭代为优先。只有 P0 级问题才立即暂停当前功能推进。

P0 仅指会破坏继续推进基础的问题，包括：

- 核心逻辑错误；
- 基础架构或关键契约设计错误；
- 严重数据或安全风险；
- 继续开发会明显建立在错误基础上并扩大返工范围。

除 P0 外：

- P1～P3 原则上不打断当前迭代；
- 问题必须记录，并在下一次迭代统一安排；
- 同一实质问题连续两个迭代仍未修复时，暂停新增功能，进入集中清理；
- “连续两个迭代”不要求每轮清空全部缺陷，只限制同一问题长期滚存；
- 是否暂停依据是否破坏继续推进的基础，不依据问题数量或标签机械决定。
...
→ 被写成完整 Executor / Flow 集成完成

工作区存在 patch
→ 被写成远端 Git commit 已包含 patch
```
