...
function applyEvent(projection, event) {
  ...
  switch (event.type) {
    case 'RUN_CREATED':
      ...
      if (payload.workflow_input_snapshot) projection.workflow_input_snapshot = clone(payload.workflow_input_snapshot)
      break
    case 'RUN_INPUTS_REBOUND':
      projection.workflow_input_snapshot = clone(payload.workflow_input_snapshot || {})
      break
    ...
    case 'RUN_REJECTED':
      ensureRunTransition(projection, 'rejected')
      projection.termination_code = payload.termination_code || 'HUMAN_REVIEW_REJECTED'
      break
    case 'RUN_FAILED':
      ensureRunTransition(projection, 'failed')
      projection.error = clone(payload.error || null)
      break
    ...
    case 'GATE_CREATED': {
      ...
      projection.node_runs[gate.node_id] = {
    ...
    case 'GATE_RESOLVED': {
      ...
      projection.node_runs[gate.node_id] = {
        ...
        node_id: gate.node_id,
        node_attempt: gate.node_attempt,
        status: gate.decision === 'approve' || gate.decision === 'submit_revision' ? 'succeeded' : gate.decision === 'reject' ? 'rejected' : 'cancelled',
        gate_id: gate.gate_id,
        outputs: clone(gate.output_artifacts || {}),
      }
    ...
    case 'NODE_RETRY_SCHEDULED': {
      ...
      if (!node || !node.node_id) throw journalError('JOURNAL_NODE_INVALID', 'NODE_RETRY_SCHEDULED requires payload.node')
      projection.node_runs[node.node_id] = { ...(projection.node_runs[node.node_id] || {}), ...clone(node), status: 'retry_waiting' }
      break
    }
    case 'NODE_STARTED': {
      const node = payload.node
