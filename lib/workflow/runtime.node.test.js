...
// FLOW-D28: this MUST NOT be require('../routes/flow'). That module requires
// express at top level, so importing statusFor from it made this entire suite
// unloadable (hard RED, 12 tests uncounted) wherever dependencies are absent.
const { statusFor } = require('./errorStatus')
...
// --- FLOW-D28 regression guards -------------------------------------------
// The fix is "this suite loads without dependencies installed". That property
// is invisible in a fully installed environment, so it cannot be verified by
// running the suite here — it has to be pinned structurally, by reading source.
// Without these two guards, re-adding the import would silently restore D28
// and nothing would turn red until someone happened to run on a bare checkout.

test('FLOW-D28: the error status table stays dependency-free', () => {
  const src = fs.readFileSync(path.join(__dirname, 'errorStatus.js'), 'utf8')
  const requires = src.replace(/^\s*\/\/.*$/gm, '').match(/\brequire\s*\(/g) || []
  // Not even node builtins: the whole point is that this module can be loaded
  // by anything, anywhere, including a checkout with no node_modules at all.
  assert.deepEqual(requires, [], 'errorStatus.js must not require anything')
})

test('FLOW-D28: this suite does not pull express in through the route module', () => {
  const src = fs.readFileSync(__filename, 'utf8').replace(/^\s*\/\/.*$/gm, '')
  assert.equal(
    /require\(\s*['"][^'"]*routes\/flow['"]\s*\)/.test(src),
    false,
    'requiring lib/routes/flow here re-creates FLOW-D28 (RED instead of skip)',
  )
})
