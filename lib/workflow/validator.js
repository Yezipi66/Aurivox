...
function validateRetryPolicy(node, errors) {
  if (node.retry_policy === undefined) return
  if (!isObject(node.retry_policy)) {
    errors.push(issue('RETRY_POLICY_INVALID', `Node '${node.id}' retry_policy must be an object`, { node_id: node.id, path: `nodes.${node.id}.retry_policy` }))
    return
  }
  if (node.retry_policy.max_attempts !== undefined && (!Number.isInteger(node.retry_policy.max_attempts) || node.retry_policy.max_attempts < 1 || node.retry_policy.max_attempts > 10)) {
    errors.push(issue('RETRY_POLICY_ATTEMPTS_INVALID', `Node '${node.id}' retry_policy.max_attempts must be an integer in [1,10]`, { node_id: node.id, path: `nodes.${node.id}.retry_policy.max_attempts` }))
  }
  if (node.retry_policy.backoff_ms !== undefined && (!Number.isFinite(node.retry_policy.backoff_ms) || node.retry_policy.backoff_ms < 0 || node.retry_policy.backoff_ms > 300000)) {
    errors.push(issue('RETRY_POLICY_BACKOFF_INVALID', `Node '${node.id}' retry_policy.backoff_ms must be in [0,300000]`, { node_id: node.id, path: `nodes.${node.id}.retry_policy.backoff_ms` }))
  }
  // FLOW-D06: an explicit idempotency declaration is what unlocks retrying a
  // handler with side effects, so it must be a real boolean, never a truthy string.
  if (node.retry_policy.idempotent !== undefined && typeof node.retry_policy.idempotent !== 'boolean') {
    errors.push(issue('RETRY_POLICY_IDEMPOTENT_INVALID', `Node '${node.id}' retry_policy.idempotent must be boolean`, { node_id: node.id, path: `nodes.${node.id}.retry_policy.idempotent` }))
  }
}
...
function validateWorkflowDocument(workflow, options = {}) {
  ...
  for (const node of nodes) {
    ...
    validateRetryPolicy(node, errors)
...
function canonicalExecutionDocument(workflow) {
  const nodes = (workflow.nodes || []).map(node => {
    ...
    return {
      ...
      retry_policy: node.retry_policy || { max_attempts: 1, backoff_ms: 0 },
      resource_policy: {
