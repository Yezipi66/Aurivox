...
function collectNodeInputs(plan, projection, node, workflowInputs) {
  ...
  for (const edge of incomingEdges(plan, node.id)) {
    ...
    const value = sourceOutputs(projection, edge.from.node)[edge.from.port]
...
class WorkflowExecutor {
  ...
  async _settle(runId) {
    ...
    if (TERMINAL_RUN_STATES.has(projection.status)) this.runtimeInputs.delete(runId)
    return projection
  }
  ...
  async _drive(runId, plan, runtimeInputs, signal) {
    ...
    while (true) {
      ...
      for (const nodeId of plan.node_order) {
        ...
        const inputsForNode = collectNodeInputs(plan, projection, node, inputs)
        if (node.disabled) {
          await this._append(runId, {
