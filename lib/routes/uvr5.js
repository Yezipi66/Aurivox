...
//
// The download spawns pipeline/uvr5/download_uvr5.py (stdlib urllib,
// HF primary + ModelScope fallback) and parses its `PROGRESS/DONE/FAIL` lines into
// a live in-memory job. Weights land in PATHS.UVR5_WEIGHTS_DIR.

const express = require("express");
const path = require("path");
const crypto = require("crypto");
const uvr5 = require("../../pipeline/uvr5/uvr5_models");
...
module.exports = function createRouter(ctx) {
  ...
  const scriptPath = path.join(
    require("../paths").UVR5_DIR, "download_uvr5.py"
  );
  ...
  router.post("/api/uvr5/download", requireApiKey, (req, res) => {
    ...
    // --dest is mandatory, not optional: without it the downloader falls back to
    // its own default, and "where the app downloads to" and "where the app reads
    // from" become two independently maintained facts. They have drifted before.
    const args = [scriptPath, "--source", source, "--dest", weightsDir];
