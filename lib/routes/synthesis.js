...
module.exports = function createRouter(ctx) {
  ...
  const { APP_DIR, ASSETS_DIR, ASSETS_ROOT, AUDIO_FORMATS, baseVoiceReg, buildTtsPayload, checkFfmpeg, clientError, computeSegmentBounds, concatWavFiles, fs, genAssetDir, genBaseName, generateOneSegment, gsvPost, isBaseVoice, loadAdvancedParams, loadVoices, newGenId, normSource, path, pathResolver, pickBestCkpt, recipeStore, requireApiKey, resolveRefPath, resolveSeed, splitJapaneseText, switchModels, toPcm16Wav, transcodeAudio, withGenerationLock, writeGenMeta } = ctx;
  ...
  return await withGenerationLock(async () => {
      ...
      if (!shouldSplit || text.length <= softLimit) {
        ...
        return { ok: true, id: genId, voice, split: false, concat: false, audio_url: audioUrl, seed: cfg.seed };
      }
      ...
      for (let i = 0; i < segments.length; i++) {
        ...
        try {
          ...
          throw new HttpError(502, clientError(err, `Segment ${i} failed`), {
            ok: false,
            segments: segResults,
          });
        }
  ...
  if (typeof voice === "string" && voice.includes("/")) {
    ...
    // Language defense stack: recipe.language (export-time choice) > asset
    // language > "auto". A blank recipe.language means "don't freeze the
    // language" — follow the asset, and ultimately auto.
    ...
    if (!recipe.language && !voiceReg.text_lang && !voiceReg.language) {
    ...
    promptLang = voiceReg.prompt_lang || voiceReg.language || "auto";
    if (!voiceReg.text_lang && !voiceReg.language) {
  ...
  return await withGenerationLock(async () => {
        ...
        if (langWarning) headers["X-Language-Warning"] = langWarning;
