  ...
  return await withGenerationLock(async () => {
        ...
        if (wantStream) {
          ...
          for (const key of ["sample_steps", "if_sr"]) {
            if (cfg[key] !== undefined && acceptsKey(sProfile, key)) spayload[key] = cfg[key];
          }
          ...
          for (const k of ["overlap_length", "min_chunk_length"]) {
            if (!acceptsKey(sProfile, k)) continue;
            const v = (req.body || {})[k];
        ...
        for (const key of ["sample_steps", "if_sr"]) {
          if (cfg[key] !== undefined && acceptsKey(nProfile, key)) payload[key] = cfg[key];
        }
