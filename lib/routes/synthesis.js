...
module.exports = function createRouter(ctx) {
  ...
  const { model, voice, input, response_format, speed, language } = req.body || {};
  if (!voice) throw new HttpError(400, "Missing 'voice' field");
  ...
  if (typeof voice === "string" && voice.includes("/")) {
    ...
    if (reqLang) {
      ...
      if (reqLangUnsupported) {
        langWarning = `Requested language '${reqLangUnsupported}' is not supported (supported: zh, ja, en, auto - or canonical all_zh/all_ja/auto_zh_ja); text_lang fell back to 'auto'.`;
