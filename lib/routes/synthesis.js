...
module.exports = function createRouter(ctx) {
  ...
  const { APP_DIR, ASSETS_DIR, ASSETS_ROOT, AUDIO_FORMATS, baseVoiceReg, buildTtsPayload, checkFfmpeg, clientError, computeSegmentBounds, concatWavFiles, fs, genAssetDir, genBaseName, generateOneSegment, gsvPost, gsvStream, isBaseVoice, loadAdvancedParams, loadVoices, newGenId, normSource, path, pathResolver, pickBestCkpt, recipeStore, requireApiKey, resolveRefPath, resolveSeed, splitJapaneseText, switchModels, toPcm16Wav, transcodeAudio, withGenerationLock, writeGenMeta } = ctx;
  ...
  router.post("/v1/audio/speech", requireApiKey, asyncHandler(
    async (req, res) => speechService(req, res),
