...
const { findLegacyDefaultId: _findLegacyDefaultId } = require("../engines/legacyDefault");
// 名片驱动的请求体组装（契约 v2 第 1c 步）：这条路由过去直接写引擎的键名，
// 现在只问名片「你把这个概念叫什么」「你收不收这个键」。
const { engineKey, acceptsKey } = require("../engines/payload");
// 契约 C11：参数表只有一份，补键/收键都走这里，不在各路由里各写一张。
const { applyEngineKnobs, coerceIncoming } = require("../engines/paramTable");
// 第 3 步（甲）：出错时点名的是真正出事的那台引擎，不是写死的 GPT-SoVITS。
const { upstreamFailure, emptyAudioFailure, isTransportError, transportFailure } = require("../engines/upstreamError");

module.exports = function createRouter(ctx) {
  const router = express.Router();
  const resolveEngineProfile = ctx.resolveEngineProfile || _resolveEngineProfile;
  const findLegacyDefaultId = ctx.findLegacyDefaultId || _findLegacyDefaultId;
  ...
  // ⭐ 第 1c 步：这次调用发给哪台引擎、带它的哪一格参数。
  // 配方路径上由配方自己说了算（v4 的 engine_id）；整音色路径今天还没有
  // 「引擎」这个参数（那是契约 §12 第 2 步），留空 = 老路径引擎。
  let callEngine = null;
  let callEngineParams = null;
  ...
  if (typeof voice === "string" && voice.includes("/")) {
    ...
    // ---- v4: 配方自己说去哪台引擎 -----------------------------------------
    // 这是配方成为「闭合的调用目标」的最后一块：除了 text，什么都不缺。
    // 老配方（v<4）没有 engine_id，读时视图把它们认领给老路径引擎 ——
    // 这不是替它们做选择，v3 时代本来就只有一台引擎。
    // ⛔ 名片读不出来就 400，不回落：回落会把「名片写错」表现成「声音不对」。
    let recipeEngine;
    try {
      recipeEngine = resolveEngineProfile(toV4(recipe, findLegacyDefaultId()).engine_id);
    } catch (err) {
      throw new HttpError(400, `Recipe '${voice}' engine unavailable: ${(err && err.message) || err}`);
    }
    const v4 = toV4(recipe, recipeEngine.id);
    callEngine = recipeEngine;
    // 这台引擎的参数格子。⚠ 只取属于它的那一格 —— 配方里可能同时存着好几台
    // 引擎的格子（换引擎时上一台的参数不该丢），发的时候只能发这一台的。
    callEngineParams = (v4.engine_params && v4.engine_params[recipeEngine.id]) || null;
    ...
    // 参考音频/参考文本**不再无条件必填** —— 改由名片说了算。
    //
    // 我查了 6 台引擎的真实推理入口：Kokoro 根本没有参考音频这个概念（50+ 预设
    // 音色包）；XTTS-v2 有参考音频但**不接受参考文本**；F5-TTS 的参考文本可空
    // （空则自动转录）。写死必填 = 这三类引擎的配方永远存不进来也调不出去。
    // 名片不声明就不查，声明了才查 —— 下游作者零代码接引擎，靠的就是这条。
    if (recipeEngine.requires_reference_audio) {
    ...
    } else if (refAudio && !fs.existsSync(refAudio)) {
      // 名片说不需要，但配方里确实填了一个指不到文件的路径 —— 这是配方坏了，
      // 不是引擎的事，照样拦。静默把坏路径传下去只会换来一句模糊的上游报错。
      throw new HttpError(400, `Recipe '${voice}' reference_audio not found: ${JSON.stringify(recipe.reference_audio)}`);
    }
    ...
    // v4 读时视图：通用格子（speed/seed）+ 本引擎格子，合成一张平表。
    // 下面 withGenerationLock 里那一堆 pick(k, adv) 的读法一个字都不用改 ——
    // v3 配方经过视图之后形状与今天完全一致，v4 配方多出来的键也在同一张表上。
    recParams = flatParams(v4);
  } else {
  ...
  return await withGenerationLock(async () => {
        ...
        // 引擎旋钮：配方里钉了就用配方的，没钉就用 advanced_params 里的。
        //
        // 这里原本逐个列了 12 个键名 —— 那就是契约 C11 说的第二份参数表。
        // 少列一个，用户在高级设置里拧的那一格在「按配方分发」这条路上就
        // **静默失效**（症状是「同一个配方，webui 和分发出来的声音不一样」，
        // 而没有任何一条测试会红）。
        //
        // ⛔ 循环放在前面、专门字段放在后面：speed_factor 也在名片的格子里，
        //    但它在这条路上有自己的来源（配方的通用格子 speed，或本次调用
        //    传进来的 speed 参数）。顺序反了就会把每次调用的语速吃掉。
        const engineKnobs = {};
        // callEngine 为空 = 整音色路径（还没有「引擎」这个参数，见 :115）⇒ 老路径引擎。
        // ⭐ 走已解析的名片对象，不按 id 再查一次 —— 本文件的引擎解析是可注入的
        //   （ctx.resolveEngineProfile，见 :22），绕过它去问注册表会让测试里的
        //   假引擎在这一行失效。
        const knobProfile = callEngine || resolveEngineProfile(findLegacyDefaultId());
        for (const entry of (knobProfile.param_schema || [])) {
          const k = entry.name;
          // enum 用 || （空串视同没填，与搬家前 text_split_method 那行一致）；
          // 其余用 != null（false / 0 是有效值，不能被当成没填）。
          engineKnobs[k] = entry.type === "enum"
            ? (rp[k] || advParams[k])
            : (rp[k] != null ? rp[k] : advParams[k]);
        }
        ...
        // 配方里这台引擎的参数格子，原样挂上 —— 平台一个键都不看，
        // 拼请求体时整包盖到 payload 上（Owner 2026-08-23 定的规矩）。
        if (callEngineParams) cfg.engine_params = callEngineParams;
        ...
        if (wantStream) {
          ...
          // 没有配方 = 整音色那条老路径，它今天还没有「引擎」这个参数。
          // ⛔ 不写死一张 GSV 表兜底，仍然走名片解析（老路径引擎的名片）。
          const sProfile = callEngine || resolveEngineProfile(findLegacyDefaultId());
          const spayload = buildTtsPayload(input, cfg, sProfile);
          // ⭐ 第 1c 步：这几处过去直接写引擎的键名，现在一律问名片要。
          //   名片没有这个概念（maps 里没这一行 / 不在 payload_keys 上）就不发 ——
          //   硬塞过去，宽松的引擎静默忽略、严格的引擎当场 400，两种都不是我们要的。
          applyEngineKnobs(spayload, sProfile, cfg);
          // 本次请求可以临时顶掉任何一个引擎旋钮。原来这里只放行两个写死的键
          // （overlap_length / min_chunk_length），别的键在 body 里填了就被无声
          // 丢掉 —— 又一份写死的清单。类型和取值范围由名片说了算，转不成就当没填。
          for (const entry of (sProfile.param_schema || [])) {
            if (!acceptsKey(sProfile, entry.name)) continue;
            const v = coerceIncoming(entry, (req.body || {})[entry.name]);
            if (v !== undefined) spayload[entry.name] = v;
          }
          const _kSpeed = engineKey(sProfile, "speed");
          if (_kSpeed && cfg.speed_factor !== undefined) spayload[_kSpeed] = cfg.speed_factor;
          const _kMedia = engineKey(sProfile, "media_type");
          if (_kMedia) spayload[_kMedia] = sfmt.ext;        // wav | ogg
          const _kStream = engineKey(sProfile, "streaming");
          if (_kStream) spayload[_kStream] = 2;             // engine chunked streaming

          // ⛔ 这里过去是 gsvStream("/tts", spayload) —— 不带地址，也就是**永远
          //   发给老路径引擎**。配方明明写了 engine_id，请求却打到 9880 上：
          //   这是 v4「配方是闭合的调用目标」没有落到运输层的一处漏网。
          //   （第 1b 步只接了 synthesisService 那条路径，这条 /v1/audio/speech
          //   的路径当时没有引擎解析，所以没接上。）
          // ⛔ 传输层失败（引擎没起 / 地址写错）在这里也要点名 —— 见
          //   lib/engines/upstreamError.js 的 transportFailure 注释。
          //   外部调用方（SillyTavern 这类走 OpenAI 兼容接口的）从这句话
          //   判断该去查谁，说不出名字等于把人指向空气。
          let up;
          try {
            up = await gsvStream("/tts", spayload, 0,
              { baseUrl: sProfile.base_url, reqTimeout: sProfile.timeout_ms });
          } catch (err) {
            if (isTransportError(err)) {
              throw new HttpError(502,
                transportFailure(sProfile, err, { baseUrl: sProfile.base_url }).message);
            }
            throw err;
          }
          ...
          if (up.statusCode >= 400) {
            ...
            // 第 3 步（甲）：点名的是**这次真的发过去的那台**引擎（sProfile），
            // 不再一律自称 GPT-SoVITS。外部调用方（SillyTavern 这类）就是从
            // 这句话里判断该去查谁的。
            throw new HttpError(502,
        ...
        const nProfile = callEngine || resolveEngineProfile(findLegacyDefaultId());
        const payload = buildTtsPayload(input, cfg, nProfile);
        // aux resolved inside buildTtsPayload (Patch #11) - not re-injected here.
        applyEngineKnobs(payload, nProfile, cfg);
        const _nSpeed = engineKey(nProfile, "speed");
        if (_nSpeed && cfg.speed_factor !== undefined) payload[_nSpeed] = cfg.speed_factor;
        // 我们只收 WAV，转码在 broker 这边做。键名问名片要。
        const _nMedia = engineKey(nProfile, "media_type");
        if (_nMedia) payload[_nMedia] = "wav";

        // ⛔ 同上：这里过去也不带地址，配方选的引擎在运输层被丢掉了。
        // ⛔ 同上：非流式这一支也要接住传输层失败。
        let ttsRes;
        try {
          ttsRes = await gsvPost("/tts", payload,
            { signal: req._aurivoxSignal, baseUrl: nProfile.base_url, reqTimeout: nProfile.timeout_ms });
        } catch (err) {
          if (isTransportError(err)) {
            throw new HttpError(502,
              transportFailure(nProfile, err, { baseUrl: nProfile.base_url }).message);
          }
          throw err;
        }
        // 第 3 步（甲）：同上 —— 名字来自 nProfile 的名片。
