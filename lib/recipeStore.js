...
function createRecipeStore(recipesDir) {
  ...
  function normalize(payload, existing) {
    ...
    if (payload.reference_audio != null && typeof payload.reference_audio !== "string") {
      return { ok: false, error: "reference_audio must be a string path" };
    }
    // F1: use the single source of truth (isProjectRelativePath) so reference_audio
    // is guarded identically to aux_ref_audio_paths. The previous inline check only
    // tested `..`, leading `/`, and `:\` — it silently accepted the forward-slash
    // drive form `X:/…` (e.g. `D:/evil.wav`), which downstream resolveRefPath treats
    // as absolute. Delegating here closes that divergence.
    if (typeof payload.reference_audio === "string" && payload.reference_audio !== "" &&
        !isProjectRelativePath(payload.reference_audio)) {
      return { ok: false, error: "reference_audio must be a project-relative path" };
    }
    ...
    if (Array.isArray(auxRaw)) {
      for (const p of auxRaw) {
        ...
        if (!isProjectRelativePath(p)) {
          return { ok: false, error: `aux_ref_audio_paths must be project-relative: ${p}` };
        }
