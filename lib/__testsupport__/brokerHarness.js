...
// `refSamples` sizes the fixture reference clip. The default (64 samples ~= 0s)
// is enough for the OpenAI-compatible speech path, but the /api/generate
// service enforces a 3-10s reference, so suites that drive that path (Aurivox
// Flow) must ask for a realistic clip. 64000 samples @16kHz = 4.0s.
function buildApp(tmpRoot, { refSamples = 64 } = {}) {
  ...
  // `pipeline` is required here: server.js requires pipeline/uvr5/
  // uvr5_models directly, so omitting it makes the backend fail to boot with
  // MODULE_NOT_FOUND and every server-backed test skip instead of fail.
  // `engines` carries the inference tree; `vendor` the prebuilt binaries.
  for (const d of ["lib", "web", "engines", "pipeline", "vendor", "node_modules"]) {
    const src = path.join(REPO_ROOT, d);
    if (fs.existsSync(src)) linkDir(src, path.join(app, d));
  }

  // Fixture voices under the app's data directory. Runtime data lives in
  // <app>/data since the step-2 move; lib/paths.js still falls back to a
  // registry left at <app>/voices.json for installations that have not run the
  // data migration yet, but the harness must exercise the CURRENT layout.
  ...
  fs.writeFileSync(path.join(app, "data", "voices.json"), JSON.stringify(voices), "utf-8");
  ...
  for (const vid of Object.keys(voices)) {
    ...
    fs.writeFileSync(wav, minimalWav(refSamples));
...
// `extraEnv` lets a suite boot the same real server with a different opt-in
// feature set (e.g. FLOW_ENABLED=1 for the Aurivox Flow integration tests)
// without duplicating the harness.
async function startBroker({ extraEnv = null, refSamples = 64 } = {}) {
  const stub = await startStubEngine();
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "broker-"));
  const { appDir, assetsDir } = buildApp(tmpRoot, { refSamples });
  ...
  return { base_url: baseUrl, engine: stub, proc, stop, appDir, tmpRoot, logPath };
}
