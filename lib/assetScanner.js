...
function scanVoiceDir(voiceId, voiceDir) {
  ...
  const meta = {
    ...
    language: prev?.language || DEFAULT_LANGUAGE,
    prompt_lang: prev?.prompt_lang || prev?.language || DEFAULT_LANGUAGE,
    text_lang: prev?.text_lang || prev?.language || DEFAULT_LANGUAGE,
...
function detectAssetState(voiceId) {
  ...
  const M = hasFiles(path.join(voiceDir, "gpt_checkpoints"), /\.ckpt$/i)
            && hasFiles(path.join(voiceDir, "sovits_models"), /\.pth$/i);
...
function planRebuild(state, options = {}) {
  ...
  let needTrain = false;      // run preprocess+train
  ...
  // 4) Model branch.
  if (needTrain) stages.push("preprocess", "train");
  ...
  if (stages.length === 0) {
    ...
    if (modelsDeferred) {
      return {
        ...
        suggested_mode: "full",
        suggested_stages: ["preprocess", "train"],
        warnings,
      };
    }
  ...
  const pipelineDataStages = stages.filter(s => ["slice", "asr", "preprocess", "train"].includes(s));
  ...
  return {
    ...
    requires_confirmation: stages.includes("train"),
    warnings,
  };
}
