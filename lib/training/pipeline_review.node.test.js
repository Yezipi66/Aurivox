...
test("pauseAfterDenoise flows from stepOptions into status; reviewStage starts null", () => {
  const { t } = newTask({ pauseAfterDenoise: true });
  const st = t.getStatus();
  assert.equal(st.pauseAfterDenoise, true);
  assert.equal(st.reviewStage, null);
...
test("getDenoisePreview lists workDir/denoise audio as rel paths, sorted, non-audio ignored", () => {
  ...
  try {
    ...
    assert.deepEqual(out.files.map((f) => f.path), ["denoise/a.wav", "denoise/b.wav", "denoise/c.flac"]);
  } finally {
