...
function createPipeline(options) {
  ...
  const pipeline = {
    ...
    async start() {
      ...
      try {
        ...
        _addLog('error', `恢复准备失败: ${err.message}`);
        _writeJournal();
        return;
      }
      ...
      for (const stepDef of enabledSteps) {
        ...
        if (stepDef.key === 'preprocess') {
          ...
          if (!asrRanThisSession && !fs.existsSync(segPath)) {
          ...
          try {
            ...
            _addLog('info', langs.length
              ? `已捕获读音词典快照: ${langs.map(l => `${l}(${state.pronLexicon.files[l].entryCount})`).join(', ')}`
              : '读音词典快照: 无自定义词条');
          } catch (e) {
            _addLog('warn', `读音词典快照失败(忽略): ${e.message}`);
          }
        }
        ...
        try {
          ...
          _addLog('warn', `原因: ${state.failedAt.reason}`);
          // 失败时【保留】暂存目录，便于排查与恢复；assets/ 仍未被写入，不受污染
          _addLog('warn', `已保留暂存目录用于排查/恢复: ${workDir}`);
  ...
  function _prepareWorkspace() {
    ...
    if (isFork) {
      ...
      if (rerunStart) {
        _addLog('info', `分叉：只复制 ${rerunStart} 之前的产物，从 ${rerunStart} 起用新参重跑: ${srcWorkDir} → ${workDir}`);
        ...
        _addLog('info', `分叉复制暂存区（改参重跑）: ${srcWorkDir} → ${workDir}`);
      ...
      if (restartFailedStep && rerunStart) {
        _addLog('info', `重启失败步（参数已改），清理其产物: ${rerunStart}`);
        clearStepOutputs(workDir, rerunStart, (m) => _addLog('info', m));
      } else {
        _addLog('info', '续跑：保留已完成产物与训练 checkpoint（GSV 将自动断点续训）');
      }
  ...
  async function _graceIngestTranscript(segPath) {
    ...
    _addLog('warn',
      `未检测到本次 ASR 转写。将等待 ${grace}s，请把自备的 .list 或 segments.json 复制到暂存目录后再继续。` +
      ` / No ASR transcript this run. Waiting ${grace}s — copy your .list or segments.json into the staging folder.`);
    _addLog('info', `暂存目录 / Staging folder: ${workDir}`);
    _addLog('info',
      '规则：放 .list 会覆写 segments.json；放 segments.json 则直接使用；两者放其一即可。' +
    ...
    for (let remaining = grace; remaining > 0; remaining--) {
      ...
      if (hasList || fs.existsSync(segPath)) {
        _addLog('info', `检测到转写文件，提前结束等待。 / Transcript detected, ending wait early.`);
        break;
      }
      if (remaining % 5 === 0 || remaining <= 3) {
        _addLog('info', `等待中… 剩余 ${remaining}s / Waiting… ${remaining}s left`);
      }
    ...
    if (lists.length > 0) {
      try {
        ...
        _addLog('success',
          `已用自备 .list（${path.basename(lists[0])}，${segments.length} 行）生成 segments.json（覆写）。` +
          ` / Ingested user .list → segments.json (${segments.length} lines, overwritten).`);
        return;
      } catch (e) {
        _addLog('warn', `解析自备 .list 失败，回退检查 segments.json：${e.message} / Failed to parse .list; falling back to segments.json.`);
      }
    }
    if (fs.existsSync(segPath)) {
      _addLog('success', `使用用户放入的 segments.json。 / Using user-provided segments.json.`);
      return;
    }
    _addLog('warn', `宽限期结束，仍未发现 .list / segments.json，preprocess 将报缺转写错误。 / No transcript found after grace period.`);
  }
