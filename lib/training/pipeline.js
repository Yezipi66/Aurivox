...
function createPipeline(options) {
  ...
  function _prepareWorkspace() {
    ...
    if (isFork) {
      // 改参重跑：整目录复制源暂存区 → 复用上游产物，再清掉 rerunStart 及下游。
      ...
      copyWorkDir(srcWorkDir, workDir);
      // 复制来的 task.json 属于源任务，删掉避免污染本任务 journal。
      try { fs.unlinkSync(path.join(workDir, 'task.json')); } catch { /* ignore */ }
      if (rerunStart) {
        _addLog('info', `清理待重跑步骤及下游产物: ${rerunStart} →`);
...
module.exports = { createPipeline, getTask, getAllTasks, getStatusById, getLogsById, STAGING_ROOT, readTaskJournal, listRecoverable };
