...
function createPipeline(options) {
  ...
  const state = {
    ...
    // GIGO 门 1：人声提取完成后暂停，让用户试听分离结果再继续（默认开）。与
    // pauseAfterAsr 共用 awaiting_review 状态机，靠 reviewKind 区分（'denoise' 只
    // 试听音频、无文本校对；'asr' 走原有 .list 校对）。
    ...
    reviewKind: null,        // 'denoise' | 'asr' —— 当前 awaiting_review 的类型
    reviewListName: null,    // 当前可校对的 .list 文件名（仅 asr）
  ...
  const pipeline = {
    ...
    async start() {
      ...
      for (const stepDef of enabledSteps) {
        ...
        try {
          ...
          // GIGO 门 1：人声提取完成后，若勾选试听，则在此暂停（试听音频，无文本校对）。
          ...
          // P6 / GIGO 门 2：ASR 完成后，若勾选了人工校对，则在此暂停，直到 resume() 被调用。
    ...
    resume() {
      ...
      _addLog('info', state.reviewKind === 'denoise'
        ? 'Vocal extraction approved; resuming pipeline'
        : 'Manual review complete; resuming pipeline');
    ...
    // GIGO 门 1：列出人声提取产物（workDir/denoise 下的音频），供前端试听预览。
    // 音频字节复用现有 resolveReviewAudioPath + /api/train/review/:id/audio 路由。
    getDenoisePreview() {
      ...
      } catch { /* denoise 未产出/目录不存在 → 空列表 */ }
      const files = names.sort().map((n) => ({ name: n, rel: 'denoise/' + n }));
      return { status: state.status, reviewKind: state.reviewKind, files };
    },
  ...
  // P6：进入 awaiting_review 状态并挂起，等待 resume() 放行。kind 区分门类：
  //   'asr'     —— 解析 .list 供文本校对（原行为）。
  //   'denoise' —— 仅试听人声提取产物，无文本列表。
  function _pauseForReview(kind = 'asr') {
    ...
    _addLog('info', kind === 'denoise'
      ? 'Paused after vocal extraction, awaiting audio preview/approval'
      : `Paused after ASR, awaiting manual review (${state.reviewListName || 'no list'})`);
