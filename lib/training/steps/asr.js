...
async function runAsr(args, log) {
  ...
  // Engine selection. DAMO / FunASR is retired project-wide: it hangs/crashes on
  // Windows (notably for zh/yue), which was the real reason Chinese ASR kept
  // failing while ja/en worked. Force faster-whisper regardless of any stale
  // engines[...] config so Chinese never silently routes to the broken DAMO path.
  let engine = 'faster-whisper';
  const engines = (asrConfig.params && asrConfig.params.engines) || {};
  const requested = (engines[language] && engines[language].engine)
    || (engines['default'] && engines['default'].engine);
  if (requested && requested !== 'faster-whisper') {
    log(`ASR: ignoring retired engine '${requested}' for '${language}'; using faster-whisper.`);
  }
