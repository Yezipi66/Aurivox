 ...
 *
 * 完全使用项目内的 pipeline/asr/ + 项目 venv 的 Python
 */
...
const { ASR_DIR, FASTER_WHISPER_DIR } = require('../../paths');
...
async function runAsr(args, log) {
  ...
  // 脚本与权重是两个目录：ASR_DIR 放识别脚本，FASTER_WHISPER_DIR 放权重。
  // 此前这里传的是 ASR_MODELS_DIR（models/asr），而脚本自己兜底算的是它下面的
  // models/ 子目录，两个路径不一致 —— 于是同一份 large-v3 被下载了两次。
  const buildCmd = (eng) => {
    if (eng === 'funasr') {
