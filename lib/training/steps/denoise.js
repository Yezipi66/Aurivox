...
async function run(ctx, log) {
  ...
  log(`Vocal extraction: input=${inputDir}, output=${outputDir}`);
  log(`Python: ${python}`);
  ...
  if (!fs.existsSync(modelPath)) {
    throw new Error(`UVR5 model file not found: ${modelPath}`);
  }

  log(`Running UVR5: ${python} ${script} --model ${modelPath} --input ${inputDir} --output ${outputDir}`);
  ...
  try {
    ...
    if (/UVR5 DROPPED FILES/.test(stdout)) {
      ...
      log(`⚠ Some audio was skipped (separation produced no valid result; removed from the training set). See below and the log ${logPath}:`);
      log(m.slice(0, 2000));
    }
    log(`Vocal extraction done: ${stdout ? stdout.slice(-200) : 'ok'}`);
  } catch (err) {
