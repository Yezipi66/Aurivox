...
async function runS1(ctx, log) {
  ...
  log(`开始 S1 训练 (GPT): voice=${voiceId}`);
  log(`GPT 轮数: ${trainCfg.gpt_epochs}`);
...
async function runS2(ctx, log) {
  ...
  log(`开始 S2 训练 (SoVITS): voice=${voiceId}`);
  log(`SoVITS 轮数: ${trainCfg.sovits_epochs}`);
  checkFile('segments.json'); // guard: 预处理须已产出 segments
...
async function run(ctx, log) {
  ...
  if (doS1) {
    ...
    log('跳过 S1 训练 (stepOptions)');
  }
  if (doS2) {
    Object.assign(result, await runS2(ctx, log));
  } else {
    log('跳过 S2 训练 (stepOptions)');
  }
  return result;
}
