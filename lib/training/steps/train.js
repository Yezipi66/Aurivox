...
// 准备 S1 训练配置
  const s1ConfigData = {
    train: {
      ...
      epochs: trainCfg.gpt_epochs || 20,
      batch_size: resolveBatch(trainCfg.batch_size, 8),
      save_every_n_epoch: trainCfg.save_every_n_epoch ?? 1,
  ...
  // 检查预训练 S2 模型
  const pretrainedS2G = path.join(__dirname, '..', 'gsv-tools', 'pretrained',
    'v2Pro', 's2Gv2Pro.pth');
  const pretrainedS2D = path.join(__dirname, '..', 'gsv-tools', 'pretrained',
    'v2Pro', 's2Dv2Pro.pth');
  ...
  s2Config.train.epochs = trainCfg.sovits_epochs || 10;
  ...
  s2Config.train.save_every_epoch = 1;
