...
async function run(ctx, log) {
  ...
  // 4) gpt_checkpoints/ ← logs_s1/<voiceId>/ckpt/*.ckpt
  //    重命名成 <voiceId>-e<epoch>.ckpt，使 assetScanner 的 /-e(\d+)\.ckpt$/ 能解析 steps
  ...
  if (fs.existsSync(s1CkptDir)) {
    for (const f of fs.readdirSync(s1CkptDir).filter(f => f.endsWith('.ckpt'))) {
      ...
      const epoch = m ? m[1] : '0';
      copyInto(path.join(s1CkptDir, f), path.join(publishDir, 'gpt_checkpoints'), `${voiceId}-e${epoch}.ckpt`);
    }
  }
    ...
    for (const item of fs.readdirSync(s2OutputDir)) {
      ...
      if (fs.statSync(p).isDirectory()) {
        for (const f of fs.readdirSync(p)) {
          if (f.endsWith('.pth')) copyInto(path.join(p, f), sovitsDir);
        }
      } else if (item.endsWith('.pth')) {
        copyInto(p, sovitsDir);
      }
    }
  }
