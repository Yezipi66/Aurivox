...
// 音色注册表路径统一取自 lib/paths.js，不再在此处逐级上溯拼接。
const { VOICES_JSON } = require('../../paths');
...
async function run(ctx, log) {
  ...
  // T5: "Retain training workspace after publish" — opt-in escape hatch. When the
  // user ticks it, the frontend sends steps.keepStaging=true (passed through
  // verbatim as ctx.stepOptions), and we skip the automatic .staging/{taskId}
  // cleanup below so the intermediate preprocessing features survive for
  // inspection. Default (undefined/false) preserves the current auto-clean behavior.
  const keepStaging = !!(ctx.stepOptions && ctx.stepOptions.keepStaging);
  ...
  try {
    ...
    try {
      ...
      const lang = language || meta.language || 'auto';
      voices[voiceId] = {
    ...
    // 清理整个暂存目录（包括中间件）。T5: keepStaging 为真时保留，供排查/复查
    // 预处理中间产物；默认（未勾选）维持发布后自动清理的原有行为。
    if (keepStaging) {
      log(`  Retained staging workspace (keepStaging): ${workDir}`);
    } else {
      try {
