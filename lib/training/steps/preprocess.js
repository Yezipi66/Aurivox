...
async function run(ctx, log) {
  ...
  if (needPreprocess) {
    ...
    // G2PWModel 目录：单一事实来源是 lib/paths.js，这里只负责取用。
    //
    // 这里原本在两个候选目录之间挑选：gsv_code/text/G2PWModel（自包含）与项目
    // 旁边的 GPT_SoVITS/text/G2PWModel（历史遗留）。r12b 把权重统一并入 models/
    // 之后这两个目录都不复存在，于是本步骤每次都把一条死路径通过环境变量
    // g2pw_model_dir 显式传给 1-get-text.py。而显式传值会盖掉 chinese2.py 里
    // 本来正确的默认路径 —— 训练照跑、不报错，只是多音字全部退回 pypinyin。
    //
    // 权重在哪儿不在这里判断。谁需要就问 PATHS：多一处判断就多一处会过期，
    // 而过期的那一处不会报错，只会让读音悄悄变差。
    ...
    if (!hasOnnx(g2pwModelDir)) {
      log(`  ⚠ Warning: g2pW.onnx not found in ${g2pwModelDir} — polyphone disambiguation will fall back to pypinyin (no error will be raised)`);
    }
  ...
  if (_needSv) {
    ...
    if (hasSv) {
      ...
      // inference runtime (vendor/tts/gpt-sovits/infer), not in lib/inference.
