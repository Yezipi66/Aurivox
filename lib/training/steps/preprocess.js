...
async function run(ctx, log) {
  ...
  if (needPreprocess) {
    ...
    // 优先自包含目录 (gsv_code/text/G2PWModel)，回退到项目同级的外部 GPT_SoVITS。
    // 选取真正含有 g2pW.onnx 权重的目录，避免两处重复维护。
    const g2pwSelfContained = path.join(gsvCode, 'text', 'G2PWModel');
    // 从 APP_DIR 起算，不从 gsvCode 数目录层数：gsv_code 的深度会随目录调整而变，
    // 这个兜底指的始终是「项目目录旁边那份 GPT_SoVITS」。
    const g2pwLegacy = path.join(PATHS.APP_DIR, '..', 'GPT_SoVITS', 'text', 'G2PWModel');
    const hasOnnx = (dir) =>
