...
from gsv_code.module.mel_processing import spectrogram_torch, spec_to_mel_torch
from gsv_code.text import cleaned_text_to_sequence
import torch.nn.functional as F
from gsv_code.tools.my_utils import load_audio
...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, hparams, version=None, val=False):
        ...
        names5 = set([name[:-4] if name.endswith('.wav') else name for name in list(os.listdir(self.path5))])  # 去除.wav后缀
        if self.is_v2Pro:
        ...
        for audiopath in tqdm(self.audiopaths_sid_text):
            ...
            wav_path = "%s/%s" % (self.path5, audiopath)
    ...
    def get_audio(self, filename):
        if not os.path.exists(filename) and os.path.exists(filename + '.wav'):
