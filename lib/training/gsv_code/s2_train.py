...
from gsv_code.module.data_utils import (
    DistributedBucketSampler,
    TextAudioSpeakerCollate,
    TextAudioSpeakerLoader,
)
from gsv_code.module.losses import discriminator_loss, feature_loss, generator_loss, kl_loss
from gsv_code.module.mel_processing import mel_spectrogram_torch, spec_to_mel_torch
from gsv_code.module.models import (
...
def run(rank, n_gpus, hps):
    ...
    collate_fn = TextAudioSpeakerCollate(version=getattr(hps.model, "version", None))
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
    ...
    for batch_idx, data in enumerate(tqdm(train_loader)):
        if getattr(hps.model, "version", None) in {"v2Pro", "v2ProPlus"}:
        ...
        if torch.cuda.is_available():
            ...
            ssl = ssl.cuda(rank, non_blocking=False)
            ssl.requires_grad = False
            # ssl_lengths = ssl_lengths.cuda(rank, non_blocking=False)
            text, text_lengths = (
                text.cuda(
                    rank,
                    non_blocking=False,
            ...
            if getattr(hps.model, "version", None) in {"v2Pro", "v2ProPlus"}:
                sv_emb = sv_emb.cuda(rank, non_blocking=False)
        else:
    ...
    ckpt_dir = "%s/logs_s2_%s" % (hps.data.exp_dir, hps.model.version)
    os.makedirs(ckpt_dir, exist_ok=True)
    half_weights_dir = hps.train.half_weights_save_dir
    if half_weights_dir:
        os.makedirs(half_weights_dir, exist_ok=True)
