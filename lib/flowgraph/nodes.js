...
define({
  ...
  params: { engine_id: 'gpt-sovits', voice: null, base_url: null },
  handler: async ({ params }) => ({
    outputs: {
      engine: {
...
//
// The parameter list is flat and its values are the defaults, because the
// engine reads `def.params` as a plain default map. Anything left empty is
// dropped by the handler rather than sent as null: the adapter only forwards
// keys it knows, and a null it did not need is still noise in the request.
define({
  ...
  params: {
    ...
    text_split_method: null,
    speed_factor: null,
    batch_size: null,
    batch_threshold: null,
    split_bucket: null,
    fragment_interval: null,
    parallel_infer: null,
    sample_steps: null,
    if_sr: null,
    aux_ref_audio_paths: null,
  },
  handler: async ({ params }) => {
    const out = {}
