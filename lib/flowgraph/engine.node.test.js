...
test('the engine-parameters node sends only what was actually filled in', async () => {
  const def = getDefinition('io.engine_params')
  assert.ok(def, 'synthesis has an engine_params input, so something has to be able to feed it')
  assert.equal(def.outputs.params.type, 'EngineParams')

  // Every setting starts empty, and an empty setting is not sent at all: the
  // server keeps its own default. Sending null instead would overwrite it.
  for (const value of Object.values(def.params)) assert.equal(value, null)

  const { outputs } = await def.handler({
    params: { ...def.params, sample_steps: 32, speed_factor: 1.1, gpt_model: '' },
    inputs: {},
    ctx: {},
  })
  assert.deepEqual(outputs.params, { sample_steps: 32, speed_factor: 1.1 })
})

// ---------------------------------------------------------------------------
//  saving a recipe back as a voice
// ---------------------------------------------------------------------------
// A voice goes into a graph as a preset and comes out of a run as a saved
// outcome. These tests hold the second direction: the node writes only when it
// has been told what to call the voice, and it never replaces one by accident.

function saveVoiceNode() {
  const def = getDefinition('out.save_voice')
  assert.ok(def, '「保存为音色」节点必须存在')
  return def
}
...
test('a recipe can be saved back as a voice, name and all', async () => {
  const def = saveVoiceNode()
  const written = []
  const events = []
  const { outputs } = await def.handler({
    inputs: { recipe: recipeFixture() },
    params: { voice_id: 'narrator_v2', display_name: '旁白 · 第二版', language: 'zh', overwrite: false },
    node: { id: 'save_voice' },
    ctx: {
      saveVoice: async request => { written.push(request); return { id: request.id, voice: {} } },
      emit: (type, payload) => events.push({ type, ...payload }),
    },
  })

  assert.equal(outputs.voice_id, 'narrator_v2', '写入的 ID 要能被后续节点引用')
  assert.equal(written.length, 1)
  assert.equal(written[0].display_name, '旁白 · 第二版')
  assert.equal(written[0].language, 'zh')
  assert.equal(written[0].reference_audio, '/refs/narrator.wav', '参考音频属于音色，必须一起保存')
  assert.deepEqual(written[0].params, recipeFixture().params)
  assert.equal(written[0].overwrite, false, '默认不覆盖，覆盖必须是明确的选择')
  assert.ok(events.some(e => e.type === 'voice_saved' && e.voice_id === 'narrator_v2'))
})

test('saving a voice refuses quietly-wrong situations instead of writing junk', async () => {
  const def = saveVoiceNode()
  const ctx = { saveVoice: async () => ({ id: 'x' }), emit: () => {} }

  await assert.rejects(
    () => def.handler({ inputs: { recipe: recipeFixture() }, params: { voice_id: '  ' }, node: { id: 'n' }, ctx }),
    err => err.code === 'FG_VOICE_NO_ID',
  )
  await assert.rejects(
    () => def.handler({ inputs: { recipe: null }, params: { voice_id: 'ok' }, node: { id: 'n' }, ctx }),
    err => err.code === 'FG_VOICE_NO_RECIPE',
  )
  // Running without a voice store (canvas without the Workbench around it) is a
  // stated limitation, not a crash halfway through a write.
  await assert.rejects(
    () => def.handler({ inputs: { recipe: recipeFixture() }, params: { voice_id: 'ok' }, node: { id: 'n' }, ctx: { emit: () => {} } }),
    err => err.code === 'FG_VOICE_STORE_UNAVAILABLE',
  )
})

test('several recipes at once save one voice and say so', async () => {
  const def = saveVoiceNode()
  const events = []
  const written = []
  await def.handler({
    inputs: { recipe: [recipeFixture(), { ...recipeFixture(), voice: 'other' }] },
    params: { voice_id: 'first_only', overwrite: true },
    node: { id: 'save_voice' },
    ctx: {
      saveVoice: async request => { written.push(request); return { id: request.id, voice: {} } },
      emit: (type, payload) => events.push({ type, ...payload }),
  ...
  assert.equal(written.length, 1, '多条配方是多个音色，不能悄悄合并')
  assert.ok(events.some(e => e.type === 'note' && /仅使用第 1 条/.test(e.message)), '只用第一条这件事必须说出来')
})
