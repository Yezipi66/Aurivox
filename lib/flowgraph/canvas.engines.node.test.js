...
const FISHTTS = Object.freeze({
  ...
  // fish_chunk 自带说明，fish_prosody 故意不写 —— 一台引擎的名片里两种
  // 都有才正常，画布对这两种都得有交代。
  param_schema: [{
    name: 'fish_chunk',
    type: 'integer',
    label: { en: 'Chunk Length', zh: '分片长度' },
    help: { en: 'Chunk length, as documented by FishTTS itself.', zh: '分片长度，由 FishTTS 自己的名片解释。' },
  }],
})
...
const GSV = Object.freeze({
  ...
  param_schema: [{
    name: 'top_k',
    type: 'integer',
    label: { en: 'Top K', zh: 'Top K' },
    help: { en: 'Sampling candidate count.', zh: '采样候选数量。' },
  }],
})
...
// engineSchema 这条缝：下面的引擎是手里递进来的假名片，盘上没有对应目录，
// 按 id 去解析必然找不到。真机上走的是默认那条（paramTable → profile.js）。
function describe (def, engines) {
  return describeDefinition(def, { engines, engineSchema: m => m.param_schema || [] })
}
...
test('2: 名片写了说明就用名片的人话，只在后面补一句「哪几台认它」', () => {
  const d = describe(paramsDef(), [GSV, FISHTTS])
  const p = param(d, 'top_k')
  assert.match(p.help.zh, /采样候选数量/, '名片写的人话被生成文案顶掉了')
  assert.match(p.help.zh, /GPT-SoVITS/)
  assert.equal(p.label.zh, 'Top K', '格子的名字也该由名片说了算')
})

// ⭐ 这条是「零代码」这句话在文案层面的兑现。
// 搬家前，这 14 条说明写在 lib/flowgraph/docs.js 里 —— 意味着一个下游作者
// 想让自己的参数在画布上有一句人话，必须**改平台的源码**。那不是插件，
// 那是提 PR。现在说明跟着名片走，平台这边一行代码都不用动。
test('2: 第三方引擎自己名片里的说明，不改平台一行代码就能显示在画布上', () => {
  const d = describe(paramsDef(), [GSV, FISHTTS])
  const p = param(d, 'fish_chunk')
  assert.match(p.help.zh, /由 FishTTS 自己的名片解释/)
  assert.match(p.help.en, /as documented by FishTTS itself/)
  assert.equal(p.label.zh, '分片长度')
  assert.equal(p.label.en, 'Chunk Length')
  // 而平台的 docs.js 里根本没有这个键。
  assert.equal(docs.paramDoc('io.engine_params', 'fish_chunk'), null)
})

// ⛔ 这条测试是拿一个真 bug 换来的：上面所有测试都从 engineSchema 那条缝
// 递假名片进来，于是「盘上的名片怎么被读出来」根本没人走过。我一度把
// paramTable 的 engineParamKeys（返回键名字符串）当成描述表接了上去，
// 660 条测试全绿，而画布上每一个格子的说明都悄悄退回了通用文案 ——
// 界面不会报错，只会变蠢。所以这里必须有一条走**默认那条路**的测试。
test('2: 走盘上真名片的那条路（不递 engineSchema）也拿得到名片写的说明', () => {
  const d = describeDefinition(paramsDef(), { engines: engineRegistry.listEngines() })
  const p = d.params.find(x => x.name === 'top_k')
  assert.ok(p, '本机装着 GPT-SoVITS，它的 top_k 该有格子')
  assert.match(p.help.zh, /采样候选数量/,
    '说明退回了通用文案 = 平台没把名片解析出来，而这件事界面上看不出错')
  assert.equal(p.label.zh, 'Top K')
})

// 同一台引擎里没写说明的那个键，依旧老实说「平台不解释它」——
// 名片写了一半不该让另一半跟着遭殃。
test('2: 名片只给一部分键写了说明，剩下的照常回落到通用文案', () => {
  const d = describe(paramsDef(), [FISHTTS])
  assert.match(param(d, 'fish_chunk').help.zh, /由 FishTTS 自己的名片解释/)
  assert.match(param(d, 'fish_prosody').help.zh, /名片/)
  assert.ok(param(d, 'fish_prosody').help.en.includes(docs.GENERIC_ENGINE_PARAM_DOC.en.slice(0, 20)))
})
