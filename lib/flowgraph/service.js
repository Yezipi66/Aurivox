...
function engineUnionParams(nodeType, engines) {
  ...
  // 手写过说明的排前面且保持原有顺序，其余按字母序 ——
  // 让老用户的界面不会因为装了一台新引擎就整个重排。
  const documented = Object.keys((docs.NODE_DOCS[nodeType] || {}).params || {})
  const rest = [...acceptedBy.keys()].filter(k => !documented.includes(k)).sort()
  const ordered = [...documented.filter(k => acceptedBy.has(k)), ...rest]
  return ordered.map(name => {
    const labels = acceptedBy.get(name)
    const doc = docs.engineParamDoc(nodeType, name, labels.map(l => l.zh), engines.length)
    const docEn = docs.engineParamDoc(nodeType, name, labels.map(l => l.en), engines.length)
    return {
      name,
      default: null,
...
function describeDefinition(def, context = {}) {
  ...
  return {
    ...
    }).concat(def.dynamic_params === 'engine_union' ? engineUnionParams(def.type, engines) : []),
  }
}
