...
const { findLegacyDefaultId: legacyDefaultId } = require('../engines/legacyDefault')
...
// 下面这几个键虽然也写在名片的 param_keys 里，但在画布上它们不归使用者管：
// 本文件下面每次都自己把它们设成固定值，因为「一个节点 = 一段音频」是
// 画布的基本约定 —— 若让图里把 split 改回 true，broker 会在背后再切一次，
// 循环次数就不再对得上实际出了几段。
//
// 它们因此也不会出现在画布的参数格子里（service.js 读这张表来隐藏）。
// 这不是引擎名单，是平台自己的职责划分，所以写在这里而不是名片里。
const RESERVED_PARAM_KEYS = Object.freeze(['split', 'concat', 'format'])
...
function pickKnown(params, manifest) {
  ...
  for (const [key, value] of Object.entries(params || {})) {
    // 平台自己拿主意的键：既不转发，也不当作「拼错了」报错。
    // 画布压根没给过填它们的地方，能走到这一行的只有手改过的图。
...
function createSynthesizeAdapter(generateService, options = {}) {
  ...
  return async function synthesize(request) {
    ...
    // 引擎节点没写 engine_id 时，认领人是名片上写 legacy_default 的那台。
    // ⛔ 这里以前硬编码 'gpt-sovits' —— lib/ 里不许出现引擎名（契约 §6）。
    const engineId = engine.engine_id || legacyDefaultId()
    ...
    const body = Object.assign({
      ...
      // ⭐ 这一行是「画布上选了引擎却全都打到同一台」的病根所在：
      //    engineId 上面校验完就被丢掉了，body 里从来没带过它，
      //    于是无论画布选谁，请求都发到老路径那台（9880）。
      engine_id: engineId,
      // 同一份参数再挂一份「按引擎分格」的形状，给开跑前点料用。
      // 平台只看这格空不空，⛔ 不看里面有什么键、值对不对。
      engine_params: { [engineId]: known },
    })
