...
// The keys the legacy synthesis service understands. Anything a node puts in
// `engine_params` that is not on this list is dropped rather than passed on —
// the service would ignore it anyway, and silently passing junk makes a typo in
// a parameter look like it worked.
const GPT_SOVITS_KEYS = Object.freeze([
...
function pickKnown(params) {
  const out = {}
  const unknown = []
  for (const [key, value] of Object.entries(params || {})) {
    if (GPT_SOVITS_KEYS.includes(key)) out[key] = value
...
function createSynthesizeAdapter(generateService, options = {}) {
  ...
  return async function synthesize(request) {
    ...
    if (engineId !== 'gpt-sovits') {
      throw adapterError('FG_ENGINE_UNSUPPORTED',
        `这台服务器上目前只接了 gpt-sovits，图里要的是 ${engineId}`,
        { engine_id: engineId })
    }
    ...
    const { known, unknown } = pickKnown(request.engine_params)
    if (unknown.length) {
