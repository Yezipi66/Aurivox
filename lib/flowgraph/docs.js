  ...
  'io.engine_params': {
    ...
    params: {
      ...
      temperature: {
        ...
        help: { en: 'Randomness of generation. Server default is 1.0.', zh: '生成随机性，服务器默认值 1.0。' },
      },
      top_k: {
        label: { en: 'top_k', zh: 'top_k' },
        help: { en: 'Sampling candidate count. Server default is 15.', zh: '采样候选数量，服务器默认值 15。' },
      },
      top_p: {
        label: { en: 'top_p', zh: 'top_p' },
        help: { en: 'Nucleus sampling threshold. Server default is 1.0.', zh: '核采样阈值，服务器默认值 1.0。' },
      },
      repetition_penalty: {
        label: { en: 'repetition_penalty', zh: '重复惩罚' },
        help: { en: 'Penalty against repeated output. Server default is 1.35.', zh: '抑制重复输出的惩罚系数，服务器默认值 1.35。' },
      },
      text_split_method: {
        label: { en: 'text_split_method', zh: '文本切分方式' },
        help: { en: 'How long text is split before synthesis. Server default is cut5.', zh: '合成前的长文本切分方式，服务器默认值 cut5。' },
      },
      speed_factor: {
        label: { en: 'speed_factor', zh: '语速系数' },
        help: { en: 'Playback speed multiplier. Server default is 1.0.', zh: '语速倍率，服务器默认值 1.0。' },
      },
      batch_size: {
        label: { en: 'batch_size', zh: '批大小' },
        help: { en: 'Segments synthesised per batch.', zh: '每批合成的分段数量。' },
      },
      batch_threshold: {
        label: { en: 'batch_threshold', zh: '批处理阈值' },
        help: { en: 'Length-similarity threshold for batching. Server default is 0.75.', zh: '分批时的长度相似度阈值，服务器默认值 0.75。' },
      },
      split_bucket: {
        label: { en: 'split_bucket', zh: '分桶处理' },
        help: { en: 'Group segments of similar length before synthesis.', zh: '合成前将长度相近的分段分组处理。' },
      },
      fragment_interval: {
        label: { en: 'fragment_interval', zh: '分段间隔' },
        help: { en: 'Silence in seconds inserted between segments. Server default is 0.3.', zh: '分段之间插入的静音秒数，服务器默认值 0.3。' },
      },
      parallel_infer: {
        label: { en: 'parallel_infer', zh: '并行推理' },
        help: { en: 'Infer segments in parallel. Server default is on.', zh: '并行推理各分段，服务器默认开启。' },
      },
      sample_steps: {
        label: { en: 'sample_steps', zh: '采样步数' },
        help: { en: 'Sampling steps for the vocoder. Server default is 32.', zh: '声码器采样步数，服务器默认值 32。' },
      },
      if_sr: {
        label: { en: 'if_sr', zh: '超分辨率' },
        help: { en: 'Run super-resolution on the result. Server default is off.', zh: '对结果执行超分辨率处理，服务器默认关闭。' },
      },
      aux_ref_audio_paths: {
...
function engineParamDoc(nodeType, name, acceptedLabels, installedCount) {
  const handwritten = paramDoc(nodeType, name)
  const base = handwritten && handwritten.help ? handwritten.help : GENERIC_ENGINE_PARAM_DOC
  ...
  return {
    label: (handwritten && handwritten.label) || { en: name, zh: name },
    help: {
