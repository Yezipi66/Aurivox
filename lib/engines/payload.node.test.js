...
test('⚠ 平台侧仍在直接说引擎方言的地方，就这些 —— 只许变少不许变多', () => {
  ...
  // 2026-08-23（丙，还 C11 的账）：一次划掉 11 个。
  //   top_k / top_p / temperature / repetition_penalty / batch_size /
  //   batch_threshold / split_bucket / fragment_interval / parallel_infer
  //     ← cfg 里那 12 行逐个列举，已改为按名片的 params.schema 循环
  //   sample_steps / if_sr / overlap_length / min_chunk_length
  //     ← 三处「补键」写死的清单，已改为 applyEngineKnobs(名片驱动)
  //
  // ⚠ 剩下这 5 个是**另一件事**，不是这一刀的范围：
  //   text_lang / prompt_lang   语言字段，平台自己要读它做 auto 回落判断
  //   speed_factor              有专门来源（配方的通用格子 speed / 调用参数）
  //   text_split_method         只剩注释里提了一次
  //   aux_ref_audio_paths       辅助参考音频，平台要解析成绝对路径并按存在性过滤
  // 它们都是「平台确实要理解这个概念」，不是「平台抄了一张引擎参数表」。
  // 真正的修法是给平台概念起平台自己的名字（cfg 词汇表去方言化），另开一刀。
  const KNOWN_LEAKS = {
    'lib/routes/synthesis.js': [
      'text_lang', 'prompt_lang', 'speed_factor', 'text_split_method',
      'aux_ref_audio_paths',
    ],
  }
