...
function resolveBaseUrl(manifest, env) {
  ...
  //   真名片必须写全这一行 —— realManifests.node.test.js 盯着。
  ...
  const declared = required(manifest, 'default_base_url', raw,
    '平台不知道该把合成请求发到哪个地址')
  return {
    base_url: String(declared).replace(/\/+$/, ''),
    source: 'manifest',
    declared_base_url,
  }
}
...
const RUNTIME_KEYS = Object.freeze([
  ...
  'ready_endpoint', 'ready_timeout_ms', 'preload', 'verify',
])
...
function relPath(manifest, keyPath, value) {
  if (typeof value !== 'string' || !value.trim()) {
    throw profileError('ENGINE_MANIFEST_INVALID_VALUE',
      `引擎 ${manifest.id} 的名片里 ${keyPath} 必须是一段非空路径，现在写的是 ${JSON.stringify(value)}`,
      { id: manifest.id, key: keyPath, value })
  }
  // 绝对路径会让名片只在作者那台机器上成立 —— 这正是契约 C3 禁止的事。
  if (/^([A-Za-z]:[\\/]|[\\/])/.test(value)) {
    throw profileError('ENGINE_MANIFEST_INVALID_VALUE',
      `引擎 ${manifest.id} 的名片里 ${keyPath} 写成了绝对路径（${value}）—— ` +
      '名片要能跟着项目搬家，路径必须是相对的（python/cwd 相对项目根，entry 相对名片所在目录）。',
      { id: manifest.id, key: keyPath, value })
  }
  return value
}
...
function parseRuntime(manifest) {
  ...
  for (const k of Object.keys(r)) {
    if (!RUNTIME_KEYS.includes(k)) {
      throw profileError('ENGINE_MANIFEST_INVALID_VALUE',
        `引擎 ${manifest.id} 的名片里 runtime.${k} 平台不认识 —— ` +
        `认识的只有 ${RUNTIME_KEYS.join(' / ')}。` +
        '多半是拼错了（拼错被静默忽略，表现是"轮询永远超时"而名片看着完全正常）。',
...
module.exports = { resolveEngineProfile, parseParamSchema, parseRuntime }
