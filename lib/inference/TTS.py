...
from sr.audio_sr import AP_BWE
...
def speed_change(input_audio: np.ndarray, speed: float, sr: int):
    n_out = int(len(input_audio) / speed)
    x_old = np.linspace(0, len(input_audio) - 1, n_out)
    x_new = np.arange(len(input_audio))
    stretched = np.interp(x_old, x_new, input_audio.astype(np.float32))
