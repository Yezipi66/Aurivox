...
// 请求没带 engine_id = 老路径，认领人是名片上写 legacy_default 的那台。
// 名片读不出来就抛，不静默回落 —— 回落会把「名片写错」表现成「声音不对」。
let engine;
try {
  engine = engine_id ? resolveEngineProfile(engine_id) : resolveEngineProfile(findLegacyDefaultId());
} catch (err) {
...
if (clip && cfg.reference_audio && /\.wav$/i.test(cfg.reference_audio) && typeof wavDurationSec === "function") {
  ...
  if (seconds > 0 && (seconds < clip.min || seconds > clip.max)) {
    // 文案保持点名 GPT-SoVITS 的历史形状：老路径今天就是这么写的，
    // 换成 engine.label 会让 33 颗钉子里那条无故变红。第 2 步再统一。
...
// 所以这条规则对它们 100% 生效。老路径在契约 §12 第 2 步被删掉时，这个
// 条件跟着一起消失。
//
...
if (engine_id) {
  ...
  const box = boxes && typeof boxes === "object" ? boxes[engine.id] : null;
  const filled = box && typeof box === "object" && !Array.isArray(box) && Object.keys(box).length > 0;
  if (!filled) {
    throw new HttpError(400, `Engine '${engine.id}' was requested but this call carries no parameters for it (engine_params.${engine.id} is missing or empty).`);
  }
}
...
return await withGenerationLock(async () => {
    // 换权重是 GPT-SoVITS 专属动作（它有 set_gpt_weights/set_sovits_weights）。
    // 名片说自己不支持热换的引擎（IndexTTS2 就是），直接跳过 —— 否则会去敲一个
    // 根本不存在的接口，报错还长得像「合成失败」。
    if (engine.hot_swap_models) await switchModels(cfg);
    ...
    if (!shouldSplit || engineBatch || text.length <= softLimit) {
      ...
      const audioBytes = toPcm16Wav(await generateOneSegment(text, cfg, engine));
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      try {
        const audioBytes = toPcm16Wav(await generateOneSegment(segText, segmentCfg, engine));
