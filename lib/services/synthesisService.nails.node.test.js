...
test('nail: meta 的字段集合与关键取值', async () => {
  ...
  assert.deepEqual(Object.keys(meta).sort(), [
    ...
    'recipe', 'recipe_id', 'ref_audio', 'ref_text', 'reuse', 'seed', 'segments', 'sovits',
    'sovits_model', 'source', 'split', 'status', 'text', 'voice', 'voiceLabel',
  ].sort())
...
test('复用留痕: meta.reuse 如实记录哪几段是复用的', async () => {
  // 假 generateOneSegment 通过第 4 个出参汇报「这段是复用的」——
  // 让第 0、2 段命中，第 1 段真推。
  const { ctx, calls } = makeCtx()
  let n = 0
  ctx.generateOneSegment = async (text, cfg, engine, out) => {
    if (out) out.cached = (n !== 1)
    n++
    return makeWav(8000)
  }
  const { generateService } = createSynthesisService(ctx)
  await generateService({ body: body({ split: true, max_chars: 10, text: 'あ'.repeat(30) }) })

  const meta = calls.meta[0].meta
  assert.equal(meta.reuse.total_segments, meta.segments)
  assert.equal(meta.reuse.reused_segments, meta.segments - 1, '只有 1 段是真推的')
  assert.equal(meta.reuse.forced, false)
})

test('复用留痕: 一段都没复用时如实记 0（不是省略字段）', async () => {
  // ⛔ 这条钉的是「出参没人写 ⇒ undefined」那个缝：老 harness 不写 out，
  //   读到 undefined 必须落成 false/0，绝不能变成 true 或让字段消失。
  const { ctx, calls } = makeCtx()   // 默认 harness 不写 out
  const { generateService } = createSynthesisService(ctx)
  await generateService({ body: body({ split: true, max_chars: 10, text: 'あ'.repeat(30) }) })

  const meta = calls.meta[0].meta
  assert.equal(meta.reuse.reused_segments, 0)
  assert.ok(meta.reuse.total_segments > 1)
})

test('复用留痕: 勾了强制重推时 meta 记 forced:true', async () => {
  const { ctx, calls } = makeCtx()
  const { generateService } = createSynthesisService(ctx)
  await generateService({ body: body({ split: false, force_resynth: true }) })
  assert.equal(calls.meta[0].meta.reuse.forced, true)
  // 而且这个开关必须真的传到引擎调用那一层去（否则它谁也管不着）
  assert.equal(calls.generateOneSegment[0].cfg.force_resynth, true)
})

test('复用留痕: 分段那一支也记 forced（两支的 meta 形状必须一致）', async () => {
  // ⛔ 两条支路各写各的 reuse 对象，很容易只改一支。这条专钉分段支。
  const { ctx, calls } = makeCtx()
  const { generateService } = createSynthesisService(ctx)
  await generateService({ body: body({ split: true, max_chars: 10, text: 'あ'.repeat(30), force_resynth: true }) })
  const meta = calls.meta[0].meta
  assert.equal(meta.reuse.forced, true)
  assert.ok(meta.reuse.total_segments > 1, '这一趟必须真的走了分段支')
})

test('⛔ force_resynth 不进 recipe：Rerun 不该被永久钉成强制重推', async () => {
  // 它是一次性动作，不是配方的属性。存进 recipe 的话，Rerun（最该命中缓存的
  // 那个按钮 —— 同一份配方原样再跑）会永远绕开缓存。
  // 照 batch 字段的先例剥掉；现场取值照旧记在 meta.reuse.forced 上。
  const { ctx, calls } = makeCtx()
  const { generateService } = createSynthesisService(ctx)
  await generateService({ body: body({ split: false, force_resynth: true }) })
  const meta = calls.meta[0].meta
  assert.ok(!('force_resynth' in meta.recipe), 'recipe 里不该留下 force_resynth')
  assert.equal(meta.reuse.forced, true, '但这次确实强制过，meta 要查得到')
})

test('⛔⛔ force_resynth 绝不能进 payload（进了就等于把整张缓存表作废）', () => {
  // 它是平台自己的词：一台引擎都没装的时候它依然有意义。若被当成引擎参数
  // 发出去，它就会进指纹 —— 而勾选状态是持久化的（Owner 要求），等于用户
  // 勾一次就再也命中不了任何缓存，这一刀直接白做。
  //
  // ⛔ 这里**走真路**：拿盘上每一张真名片，真拼一次 payload、真过一次
  //   applyEngineKnobs，然后看键在不在。只断言「它登记在 PLATFORM_REQUEST_KEYS
  //   上」是不够的 —— 那只证明词表写对了，不证明拼装那条路真的读了词表。
  const { assembleEnginePayload } = require('../engines/payload')
  const { resolveEngineProfile } = require('../engines/profile')
  const { applyEngineKnobs, PLATFORM_REQUEST_KEYS } = require('../engines/paramTable')

  assert.ok(PLATFORM_REQUEST_KEYS.includes('force_resynth'),
    'force_resynth 必须登记在平台词表上')

  const ids = fs.readdirSync(path.join(__dirname, '..', '..', 'engines'))
    .filter((d) => !d.startsWith('_'))
  assert.ok(ids.length >= 1, '一张名片都没读到，这条守卫等于没跑')

  let checked = 0
  for (const id of ids) {
    let profile
    try { profile = resolveEngineProfile(id, {}) } catch { continue }
    const cfg = { text_lang: 'zh', top_k: 15, seed: 1, force_resynth: true }
    const payload = assembleEnginePayload({
      profile,
      canonical: { text: 'hello', text_lang: 'zh', reference_audio: '/tmp/a.wav',
        reference_text: 'hi', reference_lang: 'zh', seed: 1 },
      cfg,
      engineParams: {},
    })
    applyEngineKnobs(payload, profile, cfg)
    assert.ok(!Object.prototype.hasOwnProperty.call(payload, 'force_resynth'),
      `引擎 ${id} 的 payload 里混进了 force_resynth`)
    checked++
  }
  assert.ok(checked >= 1, '没有一台引擎真的拼出过 payload，这条守卫等于没跑')
})
