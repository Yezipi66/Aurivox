...
test('faster-whisper 权重路径只有一处算法，不得再出现第二套拼法', () => {
  const src = fs.readFileSync(
    path.join(__dirname, '..', 'vendor', 'asr', 'fasterwhisper_asr.py'), 'utf8');
...
// 以下两条守卫的对象是 PowerShell 脚本。Node 无法执行它们，因此这里只能
...
test('启动脚本检查配置时，相对路径也要按项目根解析', () => {
  const src = readScript('start.ps1')
  const body = src.slice(src.indexOf('function Repair-EngineConfig'))

  // 必须落在 param(...) 上：仅在函数体里出现 $ProjectRoot 说明不了什么，
  // 删掉参数声明后函数体照样引用它（值永远为空），检查会全部跳过。
  assert.ok(/param\([^)]*\$ProjectRoot/.test(body),
    'Repair-EngineConfig 的参数表里必须有 $ProjectRoot，否则相对路径无从解析')
  assert.ok(/Repair-EngineConfig[^\n]*-ProjectRoot/.test(src),
    '调用点必须把项目根传进去，否则参数形同虚设')
  assert.ok(/_path\$/.test(body),
    '必须按 *_path 键名判定哪些值是路径')
  assert.ok(/Join-Path \$ProjectRoot/.test(body),
    '相对路径必须与项目根拼接后再 Test-Path')

  // 反向条件：旧写法是"只检查看起来像绝对路径的值"。若某次改动把
  // 键名判定去掉、退回只认盘符，这里必须变红。
  const looksAbsoluteOnly =
    /if \(\$val -match '\^\[A-Za-z\]:\[\\\\\/\]'\) \{/.test(body)
  assert.ok(!looksAbsoluteOnly,
    '不得退回"只检查绝对路径"：全相对路径的配置会被判为健康，而它可能整份都是死路径')
})
