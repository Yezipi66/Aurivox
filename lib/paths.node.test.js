...
test('推理配置模板里的每个权重路径都指向 models/，且不含已废弃的版本段', () => {
  ...
  // v1 的两半分别躺在 gsv-v2final/ 与 v2Pro/ 里，目录名完全不提示这件事。
  // 谁要是照目录名"顺手理顺"，v1 就会静默失效，故在此钉死。
  assert.ok(tpl.includes(
    'gsv-v2final/s1bert25hz-2kh-longer-epoch=68e-step=50232.ckpt'),
    'v1 的 GPT 权重实际在 gsv-v2final/ 下，模板不得改指别处');
  assert.ok(tpl.includes('v2Pro/s2G488k.pth'),
    'v1 的 SoVITS 权重实际在 v2Pro/ 下，模板不得改指别处');
});
