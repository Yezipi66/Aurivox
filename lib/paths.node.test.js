...
test('遗留位置备忘如实指向当前真实位置', () => {
  ...
  // 备忘表登记的是权重位置，不是放脚本的 ASR_DIR —— 两者已经分家
  assert.strictEqual(P.LEGACY_LOCATIONS.asr.current, P.ASR_MODELS_DIR)
  assert.notStrictEqual(P.ASR_MODELS_DIR, P.ASR_DIR)
  assert.strictEqual(P.LEGACY_LOCATIONS.uvr5Weights.current, P.UVR5_WEIGHTS_DIR)
  assert.strictEqual(
    P.LEGACY_LOCATIONS.superRes.current,
    path.join(P.GSV_INFER_DIR, 'sr', 'AP_BWE_main', '24kto48k'),
  )
  assert.strictEqual(
    P.LEGACY_LOCATIONS.langDetect.current,
    path.join(P.GSV_PRETRAINED_DIR, 'fast_langdetect'),
  )
