...
test("flow: the Run Journal is persisted to disk and replays to the same state", async (t) => {
  ...
  // FLOW-D27 stage 2 (channel 1 + channel 2). This assertion is the REVERSE of
  // what it was, and that reversal is the point.
  //
  // It previously pinned the known gap: node OUTPUTS were persisted verbatim and
  // the legacy adapter echoes the request text into InferenceResult.metadata
  // .request, so plaintext reached the Journal by that route. It was written to
  // go red the moment anyone changed that -- forcing a decision instead of a
  // side effect. It did exactly that: stage 2 tripped it, the trip was reviewed,
  // and the new behaviour was chosen explicitly. The guard worked as designed.
  //
  // Journal-safe outputs (§4.8.1) pass through verbatim; the synth node's output
  // is not journal-safe (metadata sits outside the identity allowlist), so both
  // the text output and the adapter's metadata echo are now reduced to digests.
  //
  // STILL NOT FIXED, and not in stage 2's scope:
  //   - channel 2 at the SOURCE: the adapter still builds metadata.request.text
  //     in memory; only its Journal persistence is gone.
  //   - channel 3: GATE_CREATED review payloads are appended separately and are
  //     graph-shape dependent -- structurally untouched by this slice.
  //   - FLOW-D27 therefore remains OPEN. A clean Journal needs stage 3 too.
  assert.ok(
    !raw.includes("こんにちは"),
    "FLOW-D27 stage 2: NODE_SUCCEEDED outputs must no longer carry plaintext into the Journal",
  );
