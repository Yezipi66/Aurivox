...
// --- 训练与推理的代码及权重位置 ----------------------------------------------
// 第三方代码集中于顶层 vendor/。权重目前仍随第三方目录存放，日后按功能
// 分类外迁至顶层 models/ 时，只需改动下列数行。
...
// 上游 GPT-SoVITS 源码（第三方代码，已迁入 vendor/，请勿修改）
const GSV_CODE_DIR = envDir('GSV_CODE_DIR', path.join(VENDOR_DIR, 'gsv_code'));
// 训练工具及随附权重（第三方代码，已迁入 vendor/）
const GSV_TOOLS_DIR = envDir('GSV_TOOLS_DIR', path.join(VENDOR_DIR, 'gsv-tools'));
// 推理运行时（第三方代码，已迁入 vendor/）。我方编写的进程入口
// lib/inference/infer_server.py 仍留在 lib 下，由 INFERENCE_DIR 指向。
const GSV_INFER_DIR = envDir('GSV_INFER_DIR', path.join(VENDOR_DIR, 'gsv-infer'));
const GSV_PRETRAINED_DIR = envDir('GSV_PRETRAINED_DIR', path.join(GSV_TOOLS_DIR, 'pretrained'));
const ASR_DIR = envDir('ASR_DIR', path.join(GSV_TOOLS_DIR, 'asr'));
const UVR5_DIR = envDir('UVR5_DIR', path.join(GSV_TOOLS_DIR, 'uvr5'));
const UVR5_WEIGHTS_DIR = envDir('UVR5_WEIGHTS_DIR', path.join(UVR5_DIR, 'uvr5_weights'));

// 具体权重子目录。集中于此，便于日后按功能分类外迁至 models/。
const PRETRAINED = {
  bigvgan: path.join(GSV_PRETRAINED_DIR, 'bigvgan'),
  cnhubert: path.join(GSV_PRETRAINED_DIR, 'chinese-hubert-base'),
  bert: path.join(GSV_PRETRAINED_DIR, 'chinese-roberta-wwm-ext-large'),
  langdetect: path.join(GSV_PRETRAINED_DIR, 'fast_langdetect'),
...
// --- 遗留位置备忘 -----------------------------------------------------------
// 以下位置尚未搬迁，但按目标结构本应换到 target 处。各自的搬迁均需配套迁移动作，
// 故单独列出，以免日后误认为是疏漏。
//
// 第 2 步（运行期数据迁入 data/）已完成，原先记在此处的四项均已落位，故移除。
// 第 4 步（第三方代码迁入 vendor/）已全部完成：gsv-tools、gsv_code 与推理运行时
// gsv-infer 均已落位，故一并移除。余下各项属第 3 步（权重迁入 models/）。
const MODELS_DIR = path.join(APP_DIR, 'models');
const LEGACY_LOCATIONS = Object.freeze({
  // 权重 —— 第 3 步。体量大，需支持挂载到其它盘。
  gsvPretrained: { current: GSV_PRETRAINED_DIR, target: path.join(MODELS_DIR, 'tts') },
  asr: { current: ASR_DIR, target: path.join(MODELS_DIR, 'asr') },
  uvr5Weights: { current: UVR5_WEIGHTS_DIR, target: path.join(MODELS_DIR, 'uvr5') },
  // 音频超分权重 —— 第 3 步。随上游目录结构存放，由 sr/audio_sr.py 按自身位置查找。
  superRes: { current: path.join(GSV_INFER_DIR, 'sr', 'AP_BWE_main', '24kto48k'),
    target: path.join(MODELS_DIR, 'sr') },
});
...
module.exports = {
  ...
  WEB_DIST,
  // 训练、推理与权重
  TRAINING_DIR, INFERENCE_DIR, GSV_CODE_DIR, GSV_TOOLS_DIR, GSV_INFER_DIR,
  GSV_PRETRAINED_DIR, ASR_DIR, UVR5_DIR, UVR5_WEIGHTS_DIR, PRETRAINED,
  // 启动
