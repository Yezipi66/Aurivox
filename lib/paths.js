...
// --- 权重：尚未迁出，下一轮处理 ---------------------------------------------
// 代码迁走之后 vendor/gsv-tools/ 只剩权重，下面几行是它唯一的存留原因；
// 权重迁入 models/ 之后该目录即消失。GSV_TOOLS_DIR 这个环境变量名保留，
// 以免既有部署脚本失效。
const LEGACY_WEIGHTS_DIR = envDir('GSV_TOOLS_DIR', path.join(VENDOR_DIR, 'gsv-tools'));
const GSV_PRETRAINED_DIR = envDir('GSV_PRETRAINED_DIR', path.join(LEGACY_WEIGHTS_DIR, 'pretrained'));
// ASR 权重目录（faster-whisper-large-v3-turbo/ 与 models/ 都在其下），
// 与上面存放识别脚本的 ASR_DIR 是两回事。
const ASR_MODELS_DIR = envDir('ASR_MODELS_DIR', path.join(LEGACY_WEIGHTS_DIR, 'asr'));
const UVR5_WEIGHTS_DIR = envDir('UVR5_WEIGHTS_DIR',
  path.join(LEGACY_WEIGHTS_DIR, 'uvr5', 'uvr5_weights'));
...
// 故单独列出，以免日后误认为是疏漏。
//
// 第 2 步（运行期数据迁入 data/）与第 4 步（第三方代码迁入 vendor/ 并按工作流
// 环节分类）均已完成，原先记在此处的相应各项已移除。余下全部属第 3 步：权重
// 迁入 models/，第一层按工作流环节分，第二层才是引擎名。
const MODELS_DIR = path.join(APP_DIR, 'models');
const LEGACY_LOCATIONS = Object.freeze({
  // 权重 —— 第 3 步。体量大，且不受 git 保护，故单独一轮处理。
  gsvPretrained: { current: GSV_PRETRAINED_DIR,
    target: path.join(MODELS_DIR, 'tts', 'gpt-sovits') },
  asr: { current: ASR_MODELS_DIR, target: path.join(MODELS_DIR, 'asr') },
  uvr5Weights: { current: UVR5_WEIGHTS_DIR, target: path.join(MODELS_DIR, 'uvr5') },
  // 音频超分权重 —— 第 3 步。随上游目录结构存放，由 sr/audio_sr.py 按自身位置
  // 查找，迁出前须先给该脚本补上环境变量兜底。
  superRes: { current: path.join(GSV_INFER_DIR, 'sr', 'AP_BWE_main', '24kto48k'),
    target: path.join(MODELS_DIR, 'sr') },
  // 语种识别权重 —— 第 3 步。目前 lid.176.bin 在两处各存一份，共 125 MB，
  // 起因是 langsegmenter.py 按自身位置查找；补上兜底后可合并为一份。
  langDetect: { current: path.join(GSV_PRETRAINED_DIR, 'fast_langdetect'),
    target: path.join(MODELS_DIR, 'lang') },
});
...
module.exports = {
  ...
  LEGACY_WEIGHTS_DIR, GSV_TOOLS_DIR: LEGACY_WEIGHTS_DIR,
  GSV_PRETRAINED_DIR, ASR_MODELS_DIR, UVR5_WEIGHTS_DIR, PRETRAINED,
  // 启动
