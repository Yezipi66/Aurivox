...
所有版本变更集中记录在本文件。README 只记录当前产品说明、部署、FAQ、架构和 API，不再承载历史变更。
...
- 修复韩语和 ASR Reading proofing 的混合语言布局、Show all 灰态和 hover 提示。
- 将 Target language 的 Auto 显示统一为 `Auto`，内部值保持兼容。
- 测试入口改为 Windows/POSIX shell 无关的 Node runner。
...
- 📝 **文档**：README 本节；`README.md#faq` 增补 **Q15**（v1.0.7 对外部署选项 + 环境变量小抄）；「环境要求」ffmpeg 条与「已知限制 #4」按「WAV 单声道走 Python、ffmpeg 仅非 WAV 兜底 + 自带 ffmpeg 现对 Python 可见」重写。
...
- ✅ **文档**：README 新增「推理 · Broker 并发/流式/模型留驻」小节（含并行发生在哪一层的说明与前端 `engine_batch` 用法）并互链；`README.md#faq` 增补 **Q14**（最终用户向：流式默认不落盘、请求内并行、前端引擎批量开关、模型留驻、四个环境变量小抄）；随包附 `BROKER_streaming_and_residency.md`。
...
- ✅ **onnxruntime-gpu 版本更正**：全项目（`requirements.in/.txt`、`lib/inference/requirements.txt`、`mdxnet.py`
  报错提示、README、README FAQ）统一由 `==1.18.1` 更正为 **`==1.18.0`**（匹配随包 torch cu121 的 cuDNN 8）。
...
- ✅ **文档**：`README.md#faq` FAQ 增补 Q8–Q12（FunASR 置信度、语言不匹配、MDX-Net OOM、Roformer 配置配套、人声/训练预设管理与试听）；
  README 顶部显式引导最终用户先读 README FAQ，「已知限制」补充 MDX 显存 / Roformer 配套 / FunASR 无置信度三条并互链。
