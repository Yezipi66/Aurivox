...
FFMPEG = r"D:\AI\GPT-SoVITS-v2pro-20250604\runtime\ffmpeg.exe"

base = os.path.normpath(r"D:\voices\raven")
work_base = os.path.normpath(r"D:\Project\tts_broker_openai_compat\test_raiden")

lang_config = [
    ('zh', '中文', 'funasr'),
    ('ja', '日文', 'faster-whisper'),
    ('en', '英文', 'faster-whisper'),
    ('ko', '韩文', 'faster-whisper'),
]

for lang_code, lang_name, asr_engine in lang_config:
    src_dir = os.path.normpath(os.path.join(base, lang_name))
    if not os.path.exists(src_dir):
        print(f"SKIP: {lang_name} not found")
        continue
    
    work_dir = os.path.normpath(os.path.join(work_base, lang_name))
    wav_dir = os.path.normpath(os.path.join(work_dir, 'wav'))
    slicer_out = os.path.normpath(os.path.join(work_dir, 'slicer_opt'))
    ...
    print(f"Step 0: Converting {len(mp3_files)} mp3 to wav...")
    for f in mp3_files:
    ...
    # Step 1: Slice
    print(f"Step 1: Slicing {len(wav_files)} files...")
    ...
    if asr_engine == 'funasr':
        ...
        asr_args = ['-i', slicer_out, '-o', asr_out, '-l', lang_code]
    else:
    ...
    # Register
    assets_dir = os.path.normpath(os.path.join(r"D:\Project\tts_broker_openai_compat\assets", f"Raiden_{lang_name}"))
