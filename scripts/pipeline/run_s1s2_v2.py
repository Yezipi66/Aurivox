...
print(f"  Config: {s1_config_path}", flush=True)
print(f"  Output: {s1_output}", flush=True)

# Run s1_train.py with -c argument pointing to our config
proc = subprocess.Popen(
    [PYTHON, '-u', os.path.join(GSV_CODE, "s1_train.py"), '-c', s1_config_path],
