...
phoneme = os.path.join(work_dir, "2-name2text-0.txt")

# Use GPT-SoVITS pretrained models (compatible with our code)
MODELS = {
    "s1": os.path.join(GSV_TOOLS, "pretrained", "gsv-v2final", "s1bert25hz-2kh-longer-epoch=68e-step=50232.ckpt"),
    "s2G": os.path.join(GSV_TOOLS, "pretrained", "v2Pro", "s2G488k.pth"),
    "s2D": os.path.join(GSV_TOOLS, "pretrained", "v2Pro", "s2D488k.pth"),
}
...
s1_config["train_phoneme_path"] = phoneme
...
print(f"  S1 model: {MODELS['s1']}", flush=True)
...
print(f"  S2G model: {MODELS['s2G']}", flush=True)
print(f"  S2D model: {MODELS['s2D']}", flush=True)
