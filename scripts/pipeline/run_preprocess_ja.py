...
r = subprocess.run([PYTHON, os.path.join(GSV_CODE, "prepare_datasets", "1-get-text.py")],
    capture_output=True, text=True, timeout=600, env={**os.environ, **env1})
...
# Need to check if wav files exist in work_dir
wav_dir = os.path.join(work_dir, "wav")
slicer_dir = os.path.join(work_dir, "slicer_opt")
env2 = {
...
r = subprocess.run([PYTHON, os.path.join(GSV_CODE, "prepare_datasets", "2-get-hubert-wav32k.py")],
    capture_output=True, text=True, timeout=600, env={**os.environ, **env2})
print(f"  Exit: {r.returncode}")
if r.stdout: print(f"  stdout: {r.stdout[-200:]}")
if r.stderr: print(f"  stderr: {r.stderr[-200:]}")
...
r = subprocess.run([PYTHON, os.path.join(GSV_CODE, "prepare_datasets", "3-get-semantic.py")],
    capture_output=True, text=True, timeout=600, env={**os.environ, **env3})
print(f"  Exit: {r.returncode}")
if r.stdout: print(f"  stdout: {r.stdout[-200:]}")
if r.stderr: print(f"  stderr: {r.stderr[-200:]}")
