...
s1_config = {
    ...
    "train_semantic_path": semantic, "train_phoneme_path": name2text}
with open(os.path.join(work_dir, "s1_config.yaml"), 'w') as f: yaml.dump(s1_config, f)

print("=== Step 4: S1 Train (20 epochs) ===", flush=True)
print(f"  Config: {os.path.join(work_dir, 's1_config.yaml')}", flush=True)
print(f"  Output: {s1_output}", flush=True)
print(f"  Semantic: {semantic}", flush=True)
print(f"  Phoneme: {name2text}", flush=True)

proc = subprocess.Popen([PYTHON, '-u', os.path.join(GSV_CODE, "s1_train.py")],
