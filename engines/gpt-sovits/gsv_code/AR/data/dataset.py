...
class Text2SemanticDataset(Dataset):
    ...
    def init_batch(self):
        ...
        if leng == 0:
            raise RuntimeError(
                f"训练集为空：semantic_data_len={semantic_data_len}, phoneme_data_len={phoneme_data_len}, "
                f"not_in_phoneme={num_not_in}, deleted_too_long={num_deleted_bigger}, deleted_ps={num_deleted_ps}。"
                "请检查语言选择/ASR 文本/切片时长。"
            )
        if leng < min_num:
